...
import org.tron.trident.extra.HeaderClientInterceptor;
...
public class ApiWrapper {
    ...
    /**
     * Indirectly grpc calling -> need to add grpc header
     */
    public ApiWrapper(String grpcEndpoint, String grpcEndpointSolidity, String hexPrivateKey, String netHeader, boolean isSsl, String accessKey) {
        HeaderClientInterceptor interceptor = HeaderClientInterceptor.builder().accessKey(accessKey).networkName(netHeader).build();
        OkHttpChannelBuilder endPointBuilder = OkHttpChannelBuilder.forTarget(grpcEndpoint).intercept(interceptor);
        OkHttpChannelBuilder endPointSolidityBuilder = OkHttpChannelBuilder.forTarget(grpcEndpointSolidity).intercept(interceptor);
        channel = isSsl
                ? endPointBuilder.useTransportSecurity().build()
                : endPointBuilder.usePlaintext().build();
        channelSolidity = isSsl
                ? endPointSolidityBuilder.useTransportSecurity().build()
                : endPointSolidityBuilder.usePlaintext().build();
        blockingStub = WalletGrpc.newBlockingStub(channel);
        blockingStubSolidity = WalletSolidityGrpc.newBlockingStub(channelSolidity);
        keyPair = new KeyPair(hexPrivateKey);
    }
